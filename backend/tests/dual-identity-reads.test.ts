...
describe('Dual-Identity Read Support (Phase 1D.3)', () => {
  ...
  describe('GET /api/reservations/:id', () => {
    it('should return dual-identity fields for known reservation', async () => {
      const response = await axios.get(`${BASE_URL}/reservations/1`, {
        validateStatus: () => true,
      });
    ...
    it('should return 404 for nonexistent reservation', async () => {
      const response = await axios.get(`${BASE_URL}/reservations/999999`, {
        validateStatus: () => true,
      });
  ...
  describe('GET /api/reservations/:id/folio', () => {
    it('should return dual-identity fields in folio view', async () => {
      const response = await axios.get(`${BASE_URL}/reservations/1/folio`, {
        validateStatus: () => true,
      });
  ...
  describe('GET /api/tapechart', () => {
    it('should return dual-identity fields in tapechart reservations', async () => {
      ...
      const response = await axios.get(`${BASE_URL}/tapechart?start=${start}&end=${end}`, {
        validateStatus: () => true,
      });
  ...
  describe('GET /api/bookings/:bid', () => {
    ...
    it('should return booking metadata for valid BID', async () => {
      ...
      const response = await axios.get(`${BASE_URL}/bookings/${knownBid}`, {
        validateStatus: () => true,
      });
    ...
    it('should handle BID normalization (case-insensitive)', async () => {
      ...
      const lowerResponse = await axios.get(
        `${BASE_URL}/bookings/${knownBid.toLowerCase()}`,
        { validateStatus: () => true }
      );
      ...
      const mixedResponse = await axios.get(
        `${BASE_URL}/bookings/${knownBid.substring(0, 3).toUpperCase()}${knownBid.substring(3).toLowerCase()}`,
        { validateStatus: () => true }
      );
    ...
    it('should return 404 for nonexistent BID', async () => {
      const response = await axios.get(`${BASE_URL}/bookings/INVALIDBID`, {
        validateStatus: () => true,
      });
  ...
  describe('GET /api/bookings/:bid/reservations', () => {
    ...
    it('should return all child reservations for a booking', async () => {
      ...
      const response = await axios.get(`${BASE_URL}/bookings/${knownBid}/reservations`, {
        validateStatus: () => true,
      });
    ...
    it('should return reservations ordered by stay_sequence', async () => {
      ...
      for (let i = 1; i < reservations.length; i++) {
        expect(reservations[i].stay_sequence).toBeGreaterThanOrEqual(
          reservations[i - 1].stay_sequence
        );
      }
    });

    it('should return 404 for nonexistent BID', async () => {
      const response = await axios.get(`${BASE_URL}/bookings/INVALIDBID/reservations`, {
        validateStatus: () => true,
      });
  ...
  describe('Read-only verification', () => {
    it('should not modify database state on read operations', async () => {
      // Fetch current state
      const preResponse = await axios.get(`${BASE_URL}/reservations/1`, {
        validateStatus: () => true,
      });
      ...
      // Make multiple reads
      for (let i = 0; i < 3; i++) {
        await axios.get(`${BASE_URL}/reservations/1`, {
          validateStatus: () => true,
        });
      }
      
      // Verify data unchanged
      const postResponse = await axios.get(`${BASE_URL}/reservations/1`, {
        validateStatus: () => true,
      });
